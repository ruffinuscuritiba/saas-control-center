...
function isViewKey(v: string | null): v is ViewKey {
  return v === 'dashboard' || v === 'clientes' || v === 'modulos';
}

function GestaoPageInner() {
  const searchParams = useSearchParams();
  const initialTab = searchParams.get('tab');

  const [token, setToken] = useState<string | null>(null);
  const [checked, setChecked] = useState(false);
  const [view, setView] = useState<ViewKey>(isViewKey(initialTab) ? initialTab : 'dashboard');
  ...
  return (
    <div className="min-h-screen flex bg-[#0b0d12] text-white">
      ...
      <aside className="w-64 shrink-0 border-r border-white/10 flex flex-col bg-[#0e1015]">
        <a href="/" className="flex items-center gap-2.5 px-5 py-5 border-b border-white/10 hover:bg-white/5 transition-colors">
